...
pub fn mini_grid_power(grid: &Vec<Vec<isize>>,
                       top_left_x: usize, top_left_y: usize) -> isize {
...
pub fn main() {
    ...
    let mut best_power = ((0, 0), 0isize);
    for x in 1..grid_size - 2 {
        for y in 1..grid_size - 2 {
            let cur_power = mini_grid_power(&fuel_grid, x, y);
            if cur_power > best_power.1 {
                best_power = ((x, y), cur_power);
            }
        }
    }
